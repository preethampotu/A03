...
QUnit.test('Testing loadques1 function with several sets of inputs', function (assert) {
    assert.equal(loadques1(4),true,'Tested with a question number in given total number of questions');
    assert.equal(loadques1(-1),false, 'Tested with one negative number. ');
    assert.equal(loadques1(10),false, 'Tested with question number out of the given total number of questions');
    assert.throws(function () { loadques1(null); },/The given argument is not a number/, 'Passing in a string correctly raises an Error');
});
